...
L.DataClassification = L.GeoJSON.extend({
        ...
        switch (options.distinctionMode) {
            case 'width':
                ...
                for (var i = 0; i < classes.length; i++) {
                    ...
                    hatchclasses.push(L.hatchClass([{
                        color: options.strokeColors[0],
                        width: swMax
                    }, {color: options.strokeColors[1], width: swMin}], null, angle));
                    swMin += step;
                    swMax -= step;
                }
                break;
            case 'angle':
                ...
                for (var i = 0; i < classes.length; i++) {
                    hatchclasses.push(L.hatchClass([{
                        ...
                        color: options.strokeColors[1],
                        width: swMin
                    }], null, (altAng != 0 ? angle + altAng * i : angle)));
                }
                break;
            case 'both':
            ...
            if (typeOfFeatures == "MultiPoint" || typeOfFeatures == "Point") {
                ...
                switch (mode_point) {
                    ...
                    case 'size':
                        ...
                        if (nodata && !nodataignore) {
                            var NDsymbol = svgCreator({
                                shape: ps,
                                size: Math.min.apply(Math, radiuses),
                                color: nodatacolor
                            });
    ...
    _classify(map) {
        ...
        if (classnum > 10) {
            console.warn("Don't be silly, both map and legend will look incomprehensible. Overriding classnumber with 10.");
            classnum = 10
        }
        ; // over 10
        ...
        if (!this._legendTemplate.hasOwnProperty('highest')) {
            this._legendTemplate.highest = '{low} <'
        }
        ;
        if (!this._legendTemplate.hasOwnProperty('middle')) {
            this._legendTemplate.middle = '{low} – {high}'
        }
        ;
        if (!this._legendTemplate.hasOwnProperty('lowest')) {
            this._legendTemplate.lowest = '< {high}'
        }
        ;
        if (!this._legendTemplate.hasOwnProperty('nodata')) {
            this._legendTemplate.nodata = 'No data'
        }
        ;
        ...
        if (classnum > 2 && classnum < this._features.length) {
            ...
            if (classrounding != null) {
                this._classPostProc_rounding(classrounding);
            }    // round class boundary values
        } else {
